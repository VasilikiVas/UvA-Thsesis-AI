...
class ABO(data.Dataset):
    def __init__(self, config):
        ...
        self.train_transform = config.train_transform
        self.id_map_addr = os.path.join(config.DATA_PATH, 'taxonomy.json')
        self.image_addr = config.IMAGE_PATH
        ...
        self.file_list = []
        for id_dict in self.id_map:
          cur_dict = self.id_map[id_dict]
          image_id = cur_dict["image_id"]
          pointcl_id = id_dict+".npy"
          caption = "a picture of "+ cur_dict["color"] + " " + cur_dict["material"] + " " + cur_dict["label"]
          self.file_list.append({
                'image_id': image_id,
                'pointcl_id': pointcl_id,
                'caption': caption
            })
        ...
        print_log(f'[DATASET] sample out {self.sample_points_num} points', logger='ABO')
        print_log(f'[DATASET] Open file {self.data_list_file}', logger='ABO')
        with open(self.data_list_file, 'r') as f:
            lines = f.readlines()
        if self.whole:
            with open(test_data_list_file, 'r') as f:
                test_lines = f.readlines()
            print_log(f'[DATASET] Open file {test_data_list_file}', logger='ABO')
            lines = test_lines + lines

        print_log(f'[DATASET] {len(self.file_list)} instances were loaded', logger='ABO')
    ...
    def __getitem__(self, idx):
        ...
        data = IO.get(os.path.join(self.pc_path, sample["pointcl_id"])).astype(np.float32)
        ...
        picked_image_addr = os.path.join(self.image_addr, sample["image_id"])
        ...
        return sample['image_id'], sample['pointcl_id'], tokenized_captions, data, image
...
class Dataset_3D():
    def __init__(self, args, tokenizer, dataset_type, train_transform=None):
        ...
        self.pretrain_dataset_prompt = None
